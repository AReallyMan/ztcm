...
class ExcelPipeline(object):
    ...
    def open_spider(self, spider):
        ...
        if 'win' in sys.platform:
            self.exportpath = os.path.join(os.path.abspath(os.path.join(os.getcwd(), "../..")), 'mongo_exportfile/' + spider.start_time + '---' + spider.end_time)
                ...
                try:
                    ...
                    for row in colldata.find():
                        ...
                        if self.judge_date_range(spider.start_time, spider.end_time, row['day']):
                            ...
                            ws.append(line)
                            wb.save(exportfile)
                    print(exportfile, '已经成功执行beijingShunyiSpider', 'beijingShunyiSpider')
                ...
                try:
                    ...
                    for row in colldata.find():
                        ...
                        if self.judge_date_range(spider.start_time, spider.end_time, row['day']):
                            ...
                            ws.append(line)
                            wb.save(exportfile)
                    print(exportfile, '已经成功执行ccgp', 'ccgp')
                except Exception as e:
                    print(e)
                finally:
                    wb.close()
    ...
    def judge_date_range(self, start_time, end_time, item_date):
        ...
        s_time = int(time.mktime(time.strptime(start_time, "%Y-%m-%d")))
        e_time = int(time.mktime(time.strptime(end_time, "%Y-%m-%d")))
