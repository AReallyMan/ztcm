...
class NewpaperSpider(scrapy.Spider):
    ...
    def getMsg(self, response):
        ...
        '''
        item = MiyunItem()
        # 页面通过xpath获取icon链接，它是通过截取页面链接拼装的iconurl
        for path in ["//div[@id='mainText']/p/img/@src", "//div[@id='mainText']/div/p/img/@src", "//div[@class='mainTextBox']/div/img/@src", "//div[@class='mainTextBox']/div/span/img/@src"]:
            if response.xpath(path):
                item['ld_icon'] = str(response.url).split("/t")[0] + '/' + str(response.xpath(path).extract_first()).replace("./", '')
